...
describe('VodFetchController', () => {
  ...
  describe('with SBR strategy', () => {
    ...
    it('fetches initial buffer capped at 4 GOPs with SBR', () => {
      const controller = new VodFetchController({
        framerate: 30,
        gopDurationMs: 2000,
        totalGroups: 100,
      ...
      expect(fetchRequests.length).toBe(1);
      // min(ceil(30/2), 4) = 4 GOPs (groups 0-3)
      expect(fetchRequests[0].startGroup).toBe(0);
      expect(fetchRequests[0].endGroup).toBe(3);
    });

    it('starts playback after initialBufferSec worth of frames with SBR', () => {
      const controller = new VodFetchController({
        framerate: 30,
        gopDurationMs: 2000,
        totalGroups: 100,
      ...
      // SBR initialBufferSec defaults to 3s, gopDuration 2s -> max(2, ceil(3/2)) = 2 GOPs
      // 2 GOPs * 60 frames/GOP = 120 frames needed
      // Receiving 1 GOP (60 frames) - should not be ready
      controller.onGroupReceived(0, 60);
      expect(readyToPlay).toBe(false);

      // Receive the 2nd GOP - should now be ready (120 frames >= 120 required)
      controller.onGroupReceived(1, 60);
      expect(readyToPlay).toBe(true);
    });
